...
### 通知：先把工作流做對，再談要不要發到手機

家屬與陪診員都有站內收件匣。通知內容來自**固定模板**，
呼叫端不能塞自由文字——通知會出現在鎖定畫面與瀏覽器分頁標題上，
任何病況或姓名一旦進去就收不回來。想讓家屬知道細節，就讓他點進去，
那一頁會再做一次授權檢查。

**系統本輪不會發送任何 LINE、簡訊或 Email。** 沒有已核准的 provider，
所以 outbox 一律停在「未設定」，而且資料庫層直接擋下任何「已送出」狀態。
未來要開通，需要產品、營運與法務先確認通道、opt-in 文案與最小內容。

### 回饋：三個條件都成立才問

服務已完成 **且** 小結已發布 **且** 這個人對這筆服務有閱覽授權。
付款人不會因為付了錢就自動收到邀請——付款不是授權。

一位家屬對一筆服務只填一次，資料庫唯一索引保證重試與並發都不會產生第二筆。
回饋**不公開**、不變成網站評價、不做人員排行。

### 意見／申訴：有人接手，而且提出者看得到進度

`open → acknowledged → resolved → closed`，結案後不可重開。
提出者只看得到類別、狀態、提出時間與處理結果代碼——
內部備註與負責人是內部處理過程，不是對申訴人的回覆。

遇到醫療、法律或高風險內容，介面只引導轉往院方與營運 SOP，
系統不對事實、責任或醫療結果做任何判斷。

### 品質覆核：對流程，不對人

五項 checklist（事件完整性、紀錄準時、小結清楚、授權正確、溝通完成），
可產生改善事項指派給陪診員。陪診員看到的是「要改什麼、何時前完成」，
**看不到**督導備註、家屬回饋原文或分數。

改善事項不會自動產生人事處分，也不會自動限制排班。前台文案明講：
「這些是流程上可以調整的地方，不是懲處，也不會影響您的派工資格。」

### 指標：樣本不足就不給數字

平均分需要至少 5 份回饋才顯示。少於這個數字，一個平均分可以反推到
個別家庭說了什麼。刻意**沒有**個別陪診員的評分或排行——
用服務對象的評分排人員名次，會讓接到困難個案的人分數變差，
那不是品質問題，是個案難度問題。

### 上線檢核：不能打勾

`/admin/care/release-readiness` 的每一項都從真實設定與資料算出來，
**沒有手動標記為完成的功能**。能打勾的檢核表只會讓人安心地上線，
然後在正式環境才發現條款還是空的。

人工待決項目（保留政策、保險、SOP、時效、報酬模型、外部通道）
一律標為待處理，因為程式無從判斷這些有沒有被確認過。

### 政策版本：系統不代寫條款

只提供版本、發布、接受紀錄的管理能力。正文由營運與法務貼入，
系統不撰寫、不檢查法律效力。正文空白不可發布，已發布正文不可改寫
（要改就建新版本）。

**接受政策 ≠ 單筆服務授權 ≠ 長期 consent ≠ 外部通知 opt-in**，
四件事互相獨立，任一件都不會自動產生另一件。

### 資料保留：是待辦清單，不是刪除工具

本輪**不刪除、不匿名化、不匯出**任何真實資料。只記錄「哪些資料
該在什麼時候被檢視」。真正的刪除要等法務、營運與備份策略確認後另做。

### 明確不做的事

外部訊息 provider、背景 worker／cron／輪詢、照片與附件、實際付款、
銀行帳戶、稅務、發票、退款、公開評論、人員排行、AI 摘要或建議、
醫療診斷與病歷。
