...
export default function UsersPage() {
  ...
  const [form, setForm] = useState({ name: '', account: '', password: '', role_id: '', is_active: true })
  ...
  const handleSave = async () => {
    ...
    const body: any = { name: form.name, account: form.account, role_id: parseInt(form.role_id), is_active: form.is_active }
    ...
    if (d.success) { toast.success(editing ? '已更新' : '帳號已新增'); fetchAll(); setForm({ name: '', account: '', password: '', role_id: '', is_active: true }); setEditing(null) }
  ...
  const startEdit = (u: any) => { setEditing(u.id); setForm({ name: u.name, account: u.account, password: '', role_id: String(u.role_id), is_active: u.is_active }) }
      ...
      <div className="card p-5 mb-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
          ...
          <div>
            ...
            <select className="form-input" value={form.role_id} onChange={e => setForm(f => ({ ...f, role_id: e.target.value }))}>
        ...
        <div className="flex gap-3">
          ...
          {editing && <button onClick={() => { setEditing(null); setForm({ name: '', account: '', password: '', role_id: '', is_active: true }) }} className="btn-secondary py-2 px-6 text-base">取消</button>}
        </div>
      </div>
        ...
        {loading ? <div className="py-12 text-center text-gray-400">載入中...</div> : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50"><tr>{['姓名','帳號','角色','狀態','操作'].map(h => <th key={h} className="text-left px-4 py-3 font-semibold text-gray-600">{h}</th>)}</tr></thead>
