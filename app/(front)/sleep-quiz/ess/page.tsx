    ...
    <div className="min-h-screen bg-gradient-to-b from-cyan-50 via-blue-50 to-white">
      <div className="max-w-2xl mx-auto px-4 py-8 pb-20">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <p className="text-[13px] text-gray-600 font-medium">ESS 愛潑沃斯嗜睡量表</p>
            ...
            {ESS_QUESTIONS.map((q, i) => (
              <div key={q.id}>
                <p className="font-bold text-gray-800 text-sm mb-2 flex items-start gap-2">
                  <span className="w-6 h-6 bg-cyan-100 text-cyan-700 rounded-full flex-shrink-0 flex items-center justify-center text-[13px] font-bold mt-0.5">
                    {i + 1}
                  </span>
                  ...
                  {ESS_OPTIONS.map(opt => {
                    ...
                    return (
                      <button key={opt.value} onClick={() => setAnswers(a => ({ ...a, [q.id]: opt.value }))}
                        className={`px-2 py-2 rounded-xl border-2 text-[13px] sm:text-sm font-semibold transition-all
        ...
        <button onClick={handleSubmit} disabled={!allAnswered || snoring === null || submitted}
          className="w-full bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-700 hover:to-blue-700 disabled:from-gray-200 disabled:to-gray-200 disabled:text-gray-600 text-white font-bold text-lg py-4 rounded-2xl transition-all shadow-lg shadow-cyan-200">
